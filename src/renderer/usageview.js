...
var HIDE = "(function(){function h(){try{var leaf=null,all=document.body.querySelectorAll('a,span,div,button,p,h1,h2');for(var i=0;i<all.length;i++){if(all[i].children.length===0){var t=(all[i].textContent||'').trim();if(t==='Back to Agents'){leaf=all[i];break;}}}if(!leaf)return false;var n=leaf,target=null;for(var k=0;k<14&&n&&n!==document.body;k++){var r=n.getBoundingClientRect();if(r.height>window.innerHeight*0.5&&r.width>0&&r.width<460&&r.left<90){target=n;}n=n.parentElement;}if(!target)return false;target.style.setProperty('display','none','important');target.style.setProperty('width','0','important');target.style.setProperty('min-width','0','important');target.style.setProperty('flex','0 0 0','important');var par=target.parentElement;if(par){var d=(getComputedStyle(par).display||'');if(d.indexOf('grid')>=0){par.style.setProperty('grid-template-columns','1fr','important');par.style.setProperty('gap','0','important');}}return true;}catch(e){}return false;}h();var c=0,iv=setInterval(function(){c++;h();if(c>40)clearInterval(iv);},500);try{var mo=new MutationObserver(function(){h();});mo.observe(document.body,{childList:true,subtree:true});setTimeout(function(){try{mo.disconnect();}catch(e){}},30000);}catch(e){}})();";
